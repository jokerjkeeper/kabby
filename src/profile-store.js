...
function normalizeProfile(p) {
  ...
  if (typeof p.cwd === 'string') p.cwd = normalizeCwd(p.cwd);
  p.provider = providers.normalizeProvider(p.provider);
  return p;
}
...
function load() {
  ...
  try {
    ...
    // 兼容舊資料：load 時 normalize cwd/provider（檔案上次寫入不一定有 normalize）
    for (const p of data.profiles) normalizeProfile(p);
    return data;
  } catch (err) {
...
function create({ name, cwd, cmd, args, provider }) {
...
function update(id, patch) {
  ...
  const allowed = ['name', 'cwd', 'cmd', 'args', 'provider', 'lastUsedAt', 'lastSessionId'];
  for (const k of allowed) {
    if (Object.prototype.hasOwnProperty.call(patch, k)) {
      data.profiles[idx][k] = patch[k];
    }
  }
  normalizeProfile(data.profiles[idx]);
  save(data);
